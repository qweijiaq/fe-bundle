...
const normalizeOptions = async (
  ...
  if (!entry || Object.keys(entry).length === 0) {
    throw new PrettyError(`No input files, try "wj-fe-bundle <your-file>" instead`);
  }
...
export async function build(_options: Options) {
  ...
  await Promise.all(
    [...(Array.isArray(configData) ? configData : [configData])].map(async (item) => {
      ...
      logger.info('CLI', `wj-fe-bundle v${version}`);

      if (config.path) {
        logger.info('CLI', `Using wj-fe-bundle config: ${config.path}`);
      }
